...
try {
    ...
    $currentOption = $tri;
    if ($dispo === 'En stock') {
        $query = "SELECT * FROM article WHERE catArt = 4 AND dispoArt=1";
    } elseif ($tri === 'Prix: élevé à faible') {
        $query = "SELECT * FROM article WHERE catArt = 4 ORDER BY prixArt DESC";
    } else {

      if (! isset($dispo))
        $query = "SELECT * FROM article WHERE catArt = 4";
        else if($dispo =='En rupture de stock'){
          $query = "SELECT * FROM article WHERE catArt = 4 AND dispoArt=0";
        }
        else{
